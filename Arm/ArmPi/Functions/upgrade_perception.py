...
class Perception():
    ...
    def process_img(self, img):
        ...
        reshape_img = cv2.resize(img, self.size, interpolation=cv2.INTER_NEAREST)
    ...
    def process_region_of_interest(self, img_lab_color):
        ...
        for color in color_range:
            if color in self.target_color:
                color_mask = cv2.inRange(img_lab_color, color_range[color][0], color_range[color][1]) # Find all values within given color range we want to analyze
